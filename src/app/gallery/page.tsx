...
export default function GalleryPage() {
  ...
  const handleUpload = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setImages(data.images || []);
      setSelectedFiles([]);
      setUploadMessage('Project photos uploaded successfully.');
      event.currentTarget.reset();
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'Unable to upload photos.');
    } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-amber-400">Gallery</p>
          <h1 className="mt-2 text-3xl font-black text-white sm:text-4xl">Our Completed Interior Work</h1>
        </div>
        ...
        <div className="w-full max-w-xl rounded-2xl border border-slate-800 bg-slate-900/70 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.4)] backdrop-blur-sm">
          <form onSubmit={handleUpload} className="space-y-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-white">Upload Project Photos</p>
                <p className="text-xs text-slate-400">Add completed residential or commercial work</p>
              </div>
            ...
            <label className="block">
              <span className="sr-only">Choose project photos</span>
              <input
                type="file"
                name="images"
                accept="image/*"
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <span className="text-xs text-slate-400">{fileCountLabel}</span>
              <button
                type="submit"
              ...
              >
                {isUploading ? 'Uploading...' : 'Upload Project Photos'}
              </button>
            </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-900/60 p-10 text-center">
          <p className="text-lg font-semibold text-white">No project photos uploaded yet.</p>
          <p className="mt-2 text-sm text-slate-400">Add your latest interior craftsmanship to showcase quality and detail.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {images.map((src, index) => {
            const imageName = src.split('/').pop() || `gallery-${index + 1}`;
